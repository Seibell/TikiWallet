...
import 'package:tiki_wallet/received_money_success.dart';

class ReceiveMoneyOfflinePage extends StatefulWidget {
  const ReceiveMoneyOfflinePage({super.key});

  @override
  State<ReceiveMoneyOfflinePage> createState() =>
      _ReceiveMoneyOfflinePageState();
}

class _ReceiveMoneyOfflinePageState extends State<ReceiveMoneyOfflinePage>
    ...
    if (wifiP2PInfo != null) {
      bool started = await _flutterP2pConnectionPlugin.startSocket(
        ...
        receiveString: (req) async {
          ...
          snack(req);
          Map<String, dynamic> transactionMap = jsonDecode(req);
          int sender = transactionMap['senderPhoneNumber'];
          int receiver = transactionMap['receiverPhoneNumber'];
          double amount = transactionMap['amount'];
          print(sender);
          print(receiver);
          print(amount);
          //receivedRoutine();
          _flutterP2pConnectionPlugin.sendStringToSocket("received");
          Navigator.push(
            context,
            MaterialPageRoute(
                builder: (context) => ReceivedMoneySuccessScreen(
                    senderPhoneNumber: sender.toString(),
                    receiverPhoneNumber: receiver.toString(),
  ...
  Future connectToSocket() async {
    if (wifiP2PInfo != null) {
      await _flutterP2pConnectionPlugin.connectToSocket(
        ...
        receiveString: (req) async {
          print("connect to socket callback");
          snack(req);
          Map<String, dynamic> transactionMap = jsonDecode(req);
          int sender = transactionMap['senderPhoneNumber'];
          int receiver = transactionMap['receiverPhoneNumber'];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Receive offline money'),
      ),
